    ...
    public int read(byte[] dest, int timeoutMillis) throws IOException {
        ...
        synchronized (mReadBufferLock) {
            ...
            if (numBytesRead <= 0) {
                ...
                //Log.e("드라이버", "numBytesRead: " + numBytesRead + "\n" + HexDump.dumpHexString(mReadBuffer) + "\n");

                if (mReadBuffer[0] == 0x7E) {
                    if (mReadBuffer[numBytesRead - 1] == 0x7F) {
                        System.arraycopy(mReadBuffer, 0, dest, 0, numBytesRead);
                        numBytesReadBkp = 0;
                        return numBytesRead;
                    } else {
                        System.arraycopy(mReadBuffer, 0, mReadDestBuffer, 0, numBytesRead);
                        numBytesReadBkp = numBytesRead;
                    }
                } else if (numBytesReadBkp != 0) {
                    if (mReadBuffer[numBytesRead - 1] == 0x7F) {
                        System.arraycopy(mReadBuffer, 0, mReadDestBuffer, numBytesReadBkp, numBytesRead);
                        nNumBytesReadBuf = numBytesReadBkp + numBytesRead;
                        System.arraycopy(mReadDestBuffer, 0, dest, 0, nNumBytesReadBuf);
                        Arrays.fill(mReadDestBuffer, 0, 255, (byte)0);
                        numBytesReadBkp = 0;
                        return nNumBytesReadBuf;
                    }
                }
                return 0;
            }
